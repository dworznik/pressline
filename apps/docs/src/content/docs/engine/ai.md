...
- Set `sellable: false` until moderation or the Customer's confirmation has run; Pressline will not sell it.
- Keep prompts and raw outputs private; only the Preview and Printfiles are public URLs.
- Generation is slow: answer 202 to the printfile request if you render on demand, or pre-render on finalize.
