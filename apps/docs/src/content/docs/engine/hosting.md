...
- **URLs are public and immutable.** A Printfile URL for a (Design, Spec Hash) pair never changes content. Put files under a content-addressed key (the sample uses `printfiles/<designId>/<specHash>.png`) in R2, Vercel Blob, S3 or your own disk; serve `Range` requests if you can (Pressline reads at most 64 KiB, and expects a PNG's `IDAT` to start inside it). Use one dot in the last path segment (`<specHash>.png`, never `name..png`): Printful's upload refuses a two-dot filename.
...
- Pressline verifies: exact size; 8-bit; non-interlaced; not palette-indexed; a declared color space that is not CMYK or grayscale; alpha per the Spec; a file under 200 MB; `specHash` echoed, `Content-Type` and size as declared. Anything else it can see but does not enforce it records as a **Deviation** and shows the Operator; a Deviation never blocks a sale.
