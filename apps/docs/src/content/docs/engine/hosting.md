...
- **Pre-render on finalise.** Fetch `GET /api/offers` from the Pressline instance: every Offer and variant comes with its Printfile Spec. Render a Printfile per Spec while the Customer is still in your app and checkout never waits; a Spec you cannot render (aspect, budget) makes that Offer ineligible via `offers`.
