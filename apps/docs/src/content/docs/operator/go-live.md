...
2. `pressline catalogue check` resolves every Offer; `pressline printfile check <url> --offer … --variant …` on a Printfile your Engine produced passes.
3. Rehearse with `demo: true` and a test Stripe key: place an order, watch it reach `submitted` then `cancelled` on the Operator View, receive the emails.
