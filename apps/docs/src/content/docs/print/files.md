...
- **Color**: sRGB, 8 bits per channel, declared with `sRGB` + `gAMA`. Embed the sRGB profile if your exporter writes one — Printful asks for it; never embed a CMYK or grayscale profile. Expect DTG prints darker and less saturated than a screen; avoid relying on pure neon.
...
- **What Pressline verifies**: exact size; 8-bit; non-interlaced; not palette-indexed; a declared color space that is not CMYK or grayscale; alpha per the Spec; a file under 200 MB. Everything else on this page is advice Pressline records as a **Deviation** when the file departs from it, and shows the Operator; it does not block a sale.
