...
- **Transparency**: `alpha: "allowed"` — transparent pixels print nothing (garment shows through); `forbidden` — the file must be opaque (flatten onto the product colour or white); `required` — reserved in the Spec for placements that need transparency; Pressline's Printful catalogue derivation today produces only `allowed` (DTG, DTF, embroidery, UV) and `forbidden`.
