...
- **Color type**: Pressline reads the PNG header, the first 64 KB: color type 4/6 or a `tRNS` chunk means alpha, and reaching `IDAT` without one means none. If the chunk table runs past 64 KB before `IDAT`, Pressline cannot tell either way and a placement whose Spec is `forbidden` or `required` rejects the file. Keep ancillary chunks (`iCCP`, `eXIf`, text) small enough that `IDAT` starts inside that window.
