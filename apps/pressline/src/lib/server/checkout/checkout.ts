...
/** Stripe wants sessions to live 30 minutes to 24 hours; ADR-0009 chose 1 hour. */
export const SESSION_TTL_MS = 60 * 60_000;

const storedPrintfile = (engine: string, designId: string, specHash: string) =>
  Effect.gen(function* () {
    const db = yield* Db;
    const rows = yield* db.all<{
      ...
      'SELECT url, sha256, content_type, width, height, bytes FROM printfiles WHERE engine = ? AND design_id = ? AND spec_hash = ?',
...
export const startCheckout = (req: CheckoutRequest, origin: string) =>
  Effect.gen(function* () {
    ...
    const printfile = yield* storedPrintfile(quote.engine, quote.designId, quote.specHash);
    if (!printfile) {
      return yield* new CheckoutUnavailable({
    ...
    const order: Order = yield* createOrder(
      {
        id: orderId,
        statusToken: token,
        engine: quote.engine,
        ...
        psp: { sessionId: session.id, sessionExpiresAt: session.expiresAt * 1000 },
      },
      session.id,
    );
