...
export const WebhooksLive = HttpApiBuilder.group(PresslineApi, 'webhooks', (handlers) =>
  handlers
    ...
    .handle('printful', ({ headers }) =>
      Effect.gen(function* () {
        const req = yield* HttpServerRequest.HttpServerRequest
        const rawBody = yield* req.text.pipe(
          Effect.mapError(() => new ProviderWebhookRejected({ message: 'unreadable body' })),
        )
        return yield* handlePrintfulWebhook(rawBody, {
          ...(headers['x-pf-webhook-signature']
            ? { signature: headers['x-pf-webhook-signature'] }
            : {}),
          ...(headers['x-pf-webhook-public-key']
            ? { publicKey: headers['x-pf-webhook-public-key'] }
            : {}),
        })
      }),
    ),
