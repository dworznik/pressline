...
const backendFrom = async (
  ...
  if (str(env, 'RENDER_BACKEND') === 'wasm') {
    ...
    const assets = platform?.env?.ASSETS as { fetch: (r: Request) => Promise<Response> } | undefined
...
export const getRuntime = (platform: App.Platform | undefined): Promise<Runtime> => {
  cached ??= (async () => {
    ...
    if (!settings.secret) throw new Error('ENGINE_SECRET is not set: the protocol cannot be served')
    const engine: Engine = {
