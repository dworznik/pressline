...
- Validation fetches only the header bytes of the Printfile URL (range request, falling back to a bounded read) and checks format, dimensions, color type and alpha against the Spec, and that `specHash` echoes the one sent. Because the read is bounded, a check can come back unseen rather than yes or no; an unseen alpha fails a placement whose Spec forbids or requires transparency. Content length is checked against `bytes`. Full-file hashing is not performed. Failures are typed and surfaced on the Storefront as "this design cannot be ordered on this product".
...
5. **Preflight** in `@pressline/cli`: bytes and a Printfile Spec in, the two tiers out (what Validation would refuse, and the Deviations), plus the `engine preflight` command driven over a temporary directory. It reaches no instance and no Engine, which is the point — it is what an Engine developer runs before either exists. `@pressline/render`'s suite runs the same function over the helper's own output, so a Deviation the helper starts writing fails our tests.
