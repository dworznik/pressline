...
- Migrations run at boot (first request per isolate) via the in-house migrator, because neither platform's deploy button runs a migration step. Each migration's statements and its `migrations` bookkeeping row go in one `batch`; the row's primary key is the concurrency guard (a racing cold start fails the insert, the batch rolls back, and it re-reads the applied set). Webhook registration is a visible post-deploy step (`pressline webhooks register` or the Operator View banner), since it needs the deployed URL.
