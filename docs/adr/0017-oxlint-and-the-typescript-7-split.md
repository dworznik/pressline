...
- `oxlint-tsgolint` (the `lint:type-aware` pass) is version-locked to the TypeScript major, so a TypeScript bump needs a matching bump here or that pass silently goes dark. It is not part of `pnpm verify`: a TypeScript bump would break the gate until `oxlint-tsgolint` catches up. It is clean as of this change (the nine findings it first reported, mostly `String(formData.get(x))` stringifying a `File` as `[object File]`, are fixed), so promoting it to a gate is a live option.
