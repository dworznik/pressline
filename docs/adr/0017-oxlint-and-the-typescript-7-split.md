...
- `oxlint-tsgolint` (the `lint:type-aware` pass) is version-locked to the TypeScript major, so a TypeScript bump needs a matching bump here or that pass silently goes dark. It is not part of `pnpm verify`.
