...
| Rate limit 120 requests per 60 s, `retry-after` on 429, with lower limits for shipping rates and the mockup generator                                                                                                                                              | **Fixed**: a 429 carries `retryAfterMs` (delta-seconds or HTTP date) and the submit schedule waits that instead of its own ramp; a wait too long for the attempt budget answers `retry_later` at once. The `X-Ratelimit-*` headers are read off every response and reach callers as `rateLimit()`, so Reconciliation's two per-Order passes hand the window back _before_ the wall, and stop on a 429 when it arrives unannounced. `catalogRefresh` stays unpaced: it resolves the configured Offers, a handful, not a queue that grows with Orders | #98, #153 |
...
| #153 | Reconciliation's provider passes had no rate-limit pacing. Split from #98. **Fixed**: the published `X-Ratelimit-*` headers are read and the passes yield before the wall, as well as stopping on a 429 |
