...
that list — not this one — is what decides #114. Only the section **Probed
with draft orders** was tested against the live API; every other statement is
either documentation or marked undocumented, and the probed rows are marked as
what they are — true of Printful on one day, not promises. Help
...
| -------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Bit depth.** Is a 16-bit PNG accepted? Printed correctly?                                                                                        | **undocumented**, probed: **not refused** (2026-09-13)                            | "bit depth", "8-bit" and "16-bit" appear nowhere: not in the API, not in the Help Center, not in a guide                                                                    |
| **Interlaced PNG, progressive JPEG**                                                                                                               | **undocumented**, probed: interlaced PNG **not refused** (2026-09-13)             | Neither word appears anywhere                                                                                                                                               |
...
| **Which size limit binds the order path**                                                                                                          | **undocumented**, probed: **61 MB processed, 213 MB never attached** (2026-09-13) | Three numbers exist — 200 MB and 20 000 px for the File library, 50 MB for the mockup generator — and none is tied to an order                                              |
...
### Probed with draft orders (#114, 2026-09-13)

Four hand-built PNGs, each carrying `sRGB`, `gAMA` and `pHYs` at 150 dpi so that
only the probed property differs from what `@pressline/render` writes, were
hosted on a public Vercel Blob store and submitted as one **v2 draft order
each** against the demo store (Bella + Canvas 3001, `front` / `dtg`, the exact
body the bridge's adapter sends). Nothing was confirmed; every draft was
canceled through the v1 route and every file deleted afterwards. Each draft was
read every 20 seconds for ten minutes through `GET /v2/orders/{id}`,
`GET /v2/orders/{id}/order-items` and v1 `GET /orders/{id}`.

| Probe                                          | Created     | What Printful did                                                                                                                                                                                                           | Settles                                                                                                                                                                                                                                                                                    |
| ---------------------------------------------- | ----------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 16-bit RGBA PNG, 1800 × 2400, 536 KB           | `200 draft` | v1 `files[0]`: `{"type":"default","status":"ok","width":1800,"height":2400,"dpi":150}` within six seconds; a 1000 × 1000 preview was generated                                                                              | **Does not refuse.** The `bit_depth` rejection stays on precaution: acceptance says nothing about the print                                                                                                                                                                                |
| Adam7 interlaced RGBA PNG, 1800 × 2400, 786 KB | `200 draft` | Same: `status: "ok"`, `1800 × 2400`, `dpi: 150`, preview generated                                                                                                                                                          | **Does not refuse.** The `interlaced` rejection stays on precaution                                                                                                                                                                                                                        |
| RGBA PNG, 3900 × 3900, **60 853 293 bytes**    | `200 draft` | `{"type":"default","status":"ok","size":60853293,"width":3900,"height":3900,"dpi":150,"mime_type":"image/png"}` within six seconds                                                                                          | **50 MB does not bind the order path.** The mockup generator's limit is its own                                                                                                                                                                                                            |
| RGBA PNG, 7300 × 7300, **213 199 938 bytes**   | `200 draft` | **Nothing.** The order reached `costs.calculation_status: "done"`, but across 26 reads over ten minutes the item's v2 `placements` stayed `[]` and its v1 `files` stayed `[]`. No error, no status, no explanation anywhere | **Over 200 MB is not refused; the file is silently absent.** Whether that draft could be confirmed with an empty item was not tested. This is the silent-alteration class the map's reject line exists for, so `too_large` at 200 MB stands on evidence, not only on the File Library page |

Two things learned on the side, both recorded in `order-lifecycle.md`'s open
questions: placement validation is **not** in the create response, and on a
processed file v2's `placements[].status` and `status_explanation` are **empty
strings**, so v1 `files[].status` is the signal that carries information. A
draft settles exactly one bit per row, refuses or does not refuse; none of
these rows says anything about what the file prints like, and the map's
decision not to buy that evidence stands.
...
| #114 | Probed (2026-09-13): 16-bit, interlaced and 61 MB drafts processed to `ok`; a 213 MB draft never attached its file and raised no error |
