...
- Otherwise the design's Preview is laid over the variant's `imageUrl` (your product photo, or Printful's): the Preview keeps the Printfile Spec's proportions and sits centered on the upper part of the photo (a fixed position in v1, tuned for a front print), with an "illustrative" caption. It is not a print proof.
- Without a product photo the Preview is shown alone.

Copy is English only in v1; every string lives in one module for a later localization.
