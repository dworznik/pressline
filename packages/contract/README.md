...
- `checkPrintfile(header, spec, facts)` — the verdict: `PrintfileInvalid` or `undefined`. Pure, and the only place the file itself is refused, so Validation, `printfile check` and an Engine's own check cannot disagree.
